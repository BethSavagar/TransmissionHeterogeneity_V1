...
p_transmission <- 0.01 # 50% probability of transmission - high to check functionality!
...
states_count <- matrix(ncol = 1+generations, nrow = 3) # 1+ generations to account for first generation

S_count <- length(which(init_state == "S"))
I_count <- length(which(init_state == "I"))
R_count <- length(which(init_state == "R"))

states_count[,1] <- c(S_count, I_count, R_count)
...
for(i in 2:ncol(state_tracker)){ # iterate over each generation of individuals
  ...
  new_state <- change_state(state_tracker, current_state, p_transmission) # use the change_state function to update the state of the individuals in the population, new_state is a vector containing disease states of all individuals in the new generation
  state_tracker[, i] <- new_state # update the state_tracker matrix with the new_state vector for the new generation
  
  S_count <- length(which(new_state == "S"))
  I_count <- length(which(new_state == "I"))
  R_count <- length(which(new_state == "R"))
  
  states_count[, i] <- c(S_count, I_count, R_count)
} 
