...
- The number of secondary cases is stored in the vector `secondary` and generated using  `rnbinom()` with the mean R and k parameters specified by `fit.cases`
- The serial interval for each secondary infection is generated using `rgamma()` with the shape and rate parameters specified but `fit.serial`
...
for(i in 1:runs){
  ...
  while(cases >0){
    ...
    # Update cases to contain the number of cases produced in this generation
    cases <- length(t.new)
    
    # Update t to contain the time-to-infection for each case in this generation
    t <- t.new
    
    #Update times, adding the time-to-infection for all cases in this generation
    times <- c(times, t.new)
  }
  
  sims[i, ] <- c(i, max(times), length(times))
